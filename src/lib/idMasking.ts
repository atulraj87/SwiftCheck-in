...
export const ID_MASKING_RULES: Record<string, MaskingRule> = {
  ...
  // NRIC (Singapore): Show first letter and last 4
  NRIC: {
    pattern: /^[STFG]\d{7}[A-Z]$/i,
    mask: (value: string) => {
      const cleaned = value.replace(/\s+/g, "").toUpperCase();
      if (cleaned.length === 9 && /^[STFG]\d{7}[A-Z]$/i.test(cleaned)) {
        const firstChar = cleaned[0];
        const lastFour = cleaned.slice(-4);
        return `${firstChar}XXXX${lastFour}`;
      }
      return defaultMask(value, 4);
    },
    visibleChars: 4,
  ...
  // PAN Card (India): Show last 4
  "PAN Card": {
    pattern: /^[A-Z]{5}\d{4}[A-Z]$/i,
    mask: (value: string) => {
      const cleaned = value.replace(/\s+/g, "").toUpperCase();
      if (cleaned.length === 10 && /^[A-Z]{5}\d{4}[A-Z]$/i.test(cleaned)) {
  ...
  // China ID Card: Show last 4
  "China ID": {
    pattern: /^\d{18}$/,
    mask: (value: string) => {
      const digits = value.replace(/\D/g, "");
      if (digits.length === 18) {
        return `XXXXXXXXXXXXXX${digits.slice(-4)}`;
      }
      return defaultMask(value, 4);
    },
    visibleChars: 4,
  ...
  // Korean ID (Resident Registration Number): Show last 4
  "Korean ID": {
    pattern: /^\d{6}-?\d{7}$/,
    mask: (value: string) => {
      const digits = value.replace(/\D/g, "");
      if (digits.length === 13) {
        return `XXXXXX-XXX${digits.slice(-4)}`;
      }
      return defaultMask(value, 4);
    },
    visibleChars: 4,
  ...
  // Japanese My Number: Show last 4
  "My Number": {
    pattern: /^\d{12}$/,
    mask: (value: string) => {
      const digits = value.replace(/\D/g, "");
  ...
  // Australian Medicare Card: Show last 4
  "Medicare Card": {
    pattern: /^\d{10}$/,
    mask: (value: string) => {
      const digits = value.replace(/\D/g, "");
  ...
  // Canadian Social Insurance Number: XXX-XXX-234
  "Social Insurance Number": {
    ...
    mask: (value: string) => {
      ...
      return defaultMask(value, 3);
    },
    visibleChars: 3,
    preserveFormat: true,
  },

  // EU National ID: Generic format for European countries
  "EU National ID": {
    mask: (value: string) => defaultMask(value, 4),
    visibleChars: 4,
  },

  // CPF (Brazil): XXX.XXX.XXX-89
  CPF: {
  ...
  // DNI (Spain/Argentina): Show last 4
  DNI: {
    pattern: /^\d{7,8}[A-Z]?$/i,
    mask: (value: string) => {
      const cleaned = value.replace(/\s+/g, "").toUpperCase();
  ...
  // Curp (Mexico): Show last 4
  CURP: {
    pattern: /^[A-Z]{4}\d{6}[HM][A-Z]{5}\d{2}$/i,
    mask: (value: string) => {
      const cleaned = value.replace(/\s+/g, "").toUpperCase();
      if (cleaned.length === 18) {
        return `XXXXXXXXXXXXXX${cleaned.slice(-4)}`;
      }
      return defaultMask(value, 4);
    },
    visibleChars: 4,
